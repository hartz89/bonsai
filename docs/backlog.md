...
| ~~D-14~~ | ~~`apply.py` overwrites an existing file with a fenced fragment~~ | — | **Fixed 2026-08-11.** First external-dogfood defect (`sharpshooter`, via `DOGFOOD-FEEDBACK.md`): two `claude-md` proposals each carried only the changed bullet, and `apply.py`'s `final = content` replaced all 80 lines of `CLAUDE.md`, then replaced that. Recovered only because the agent still had the original in context. The contract was stated correctly in `templates/proposal.md` and `promote/SKILL.md`, contradicted in spirit by `review/SKILL.md`'s "present as a diff", and enforced nowhere. Three layers now: every overwrite is copied to `.state/backups/` first; a target of ≥10 non-blank lines that would keep under half is refused without `--allow-shrink`; and the "ENTIRE resulting file" contract is stated where it's read. Deliberately **not** a superset check — a rewrite (`npm`→`pnpm`) isn't a superset, and a test asserts same-size rewrites stay allowed. Thresholds are a guess with no upstream source, registered in `sources.md` with a falsifier |
...
| D-15 | `PreCompact` is wired to `auto` only, and has no citation or test | S | The fallback trigger for long sessions (roadmap line 174) misses **manual** compaction entirely. Matcher support re-verified 2026-08-11: `PreCompact` takes `manual` and `auto`, so a second entry is the whole fix. Two gaps behind it: `PreCompact` is the only event in `hooks/hooks.json` with no row in `reference/sources.md` (invariant 1), and no assertion in `tests/run.sh` — D-06's missing round trip is why nobody noticed. Verified at the same time: it can block via exit 2 and supports `systemMessage`, both of which `retro.sh` avoids by always exiting 0 and writing nothing to stdout; note that in `etiquette.md` rule 1, since the event is now knowingly powerful rather than assumed inert. Raised by the sharpshooter dogfood (heavy subagent use, long sessions, rare `SessionEnd`) |
